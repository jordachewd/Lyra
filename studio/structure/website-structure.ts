...
/**
 * Anything `S.list().items()` accepts — list items and dividers alike. Derived
 * from the builder because sanity/structure does not re-export DividerBuilder.
 */
type StructureItem = Parameters<ReturnType<StructureBuilder['list']>['items']>[0][number]

export const websiteStructure = (S: StructureBuilder): StructureItem[] => [
  S.listItem()
    .title('Pages')
    .icon(DocumentsIcon)
    .child(
